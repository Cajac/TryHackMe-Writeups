...
Description:
Learn advanced techniques such as null, FIN, Xmas, and idle (zombie) scans, spoofing, 
in addition to FW and IDS evasion.
```

Room link: [https://tryhackme.com/room/nmap03](https://tryhackme.com/room/nmap03)
...
![TCP Packet Header](Images/TCP_Packet_Header.png)
...
![Nmap Null Scan Open](Images/Nmap_Null_Scan_Open.png)

However, we expect the target server to respond with an RST packet if the port is closed. Consequently, we can use the lack of RST response to figure out the ports that are not closed: open or filtered.

![Nmap Null Scan Closed](Images/Nmap_Null_Scan_Closed.png)
...
```bash
pentester@TryHackMe$ sudo nmap -sN 10.65.170.89

Starting Nmap 7.60 ( https://nmap.org ) at 2021-08-30 10:30 BST
Nmap scan report for 10.65.170.89
...
![Nmap FIN Scan Open](Images/Nmap_FIN_Scan_Open.png)

However, the target system should respond with an RST if the port is closed. Consequently, we will be able to know which ports are closed and use this knowledge to infer the ports that are open or filtered. It's worth noting some firewalls will 'silently' drop the traffic without sending an RST.

![Nmap FIN Scan Closed](Images/Nmap_FIN_Scan_Closed.png)
...
```bash
pentester@TryHackMe$ sudo nmap -sF 10.65.170.89
...
![Nmap Xmas Scan](Images/Nmap_Xmas_Scan.png)
...
```bash
pentester@TryHackMe$ sudo nmap -sX 10.65.170.89
...
Start the VM and load the AttackBox. Once both are ready, open the terminal on the AttackBox and use nmap to launch a FIN scan against the target VM.

#### How many ports appear as open|filtered?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ sudo nmap -sF 10.65.170.89    
[sudo] password for kali: 
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-15 08:46 CET
Nmap scan report for 10.65.170.89
...
```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ sudo nmap -sN 10.65.170.89
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-15 08:48 CET
Nmap scan report for 10.65.170.89
Host is up (0.11s latency).
Not shown: 991 closed tcp ports (reset)
PORT    STATE         SERVICE
...
![Nmap TCP Maimon Scan](Images/Nmap_TCP_Maimon_Scan.png)
...
![Nmap TCP ACK Scan](Images/Nmap_TCP_ACK_Scan.png)
...
```bash
pentester@TryHackMe$ sudo nmap -sA 10.65.160.242

Starting Nmap 7.60 ( https://nmap.org ) at 2021-08-30 10:37 BST
Nmap scan report for 10.65.160.242
Host is up (0.0013s latency).
All 1000 scanned ports on 10.65.160.242 are unfiltered
MAC Address: 02:45:BF:8A:2D:6B (Unknown)
...
After setting up the target VM `10.65.160.242` with a firewall, we repeated the ACK scan. This time, we received some interesting results. As seen in the console output below, we have three ports that aren't being blocked by the firewall. This result indicates that the firewall is blocking all other ports except for these three ports.
...
![Nmap Window Scan](Images/Nmap_Window_Scan.png)
...
```bash
pentester@TryHackMe$ sudo nmap -sW 10.65.160.242
...
![Nmap Custom Scan](Images/Nmap_Custom_Scan.png)
...
The VM received an update to its firewall ruleset. A new port is now allowed by the firewall. After you make sure that you have terminated the VM from Task 2, start the VM for this task. Launch the AttackBox if you haven't done that already. Once both are ready, open the terminal on the AttackBox and use Nmap to launch an ACK scan against the target VM.

#### How many ports appear unfiltered?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ sudo nmap -sA 10.65.160.242
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-15 09:02 CET
Nmap scan report for 10.65.160.242
Host is up (0.11s latency).
Not shown: 980 filtered tcp ports (no-response), 16 filtered tcp ports (admin-prohibited)
PORT    STATE      SERVICE
22/tcp  unfiltered ssh
25/tcp  unfiltered smtp
80/tcp  unfiltered http
...
To determine the new port you need to compare the scan results of Task 2 to the ones of this task.
...
```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ nc 10.65.160.242 443        
(UNKNOWN) [10.65.160.242] 443 (https) : Connection refused

┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ sudo nmap -sS -p443 10.65.160.242
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-15 09:06 CET
Nmap scan report for 10.65.160.242
Host is up (0.11s latency).
...
The following figure shows the attacker launching the command `nmap -S SPOOFED_IP 10.65.160.242`. Consequently, Nmap will craft all the packets using the provided source IP address `SPOOFED_IP`. The target machine will respond to the incoming packets sending the replies to the destination IP address `SPOOFED_IP`. For this scan to work and give accurate results, the attacker needs to monitor the network traffic to analyze the replies.

![Nmap_Spoofing_Scan](Images/Nmap_Spoofing_Scan.png)
...
In general, you expect to specify the network interface using `-e` and to explicitly disable ping scan `-Pn`. Therefore, instead of `nmap -S SPOOFED_IP 10.65.160.242`, you will need to issue `nmap -e NET_INTERFACE -Pn -S SPOOFED_IP 10.65.160.242` to tell Nmap explicitly which network interface to use and not to expect to receive a ping reply. It is worth repeating that this scan will be useless if the attacker system cannot monitor the network for responses.
...
![Nmap_Decoy_Scan](Images/Nmap_Decoy_Scan.png)

You can launch a decoy scan by specifying a specific or random IP address after `-D`. For example, `nmap -D 10.10.0.1,10.10.0.2,ME 10.65.160.242` will make the scan of `10.65.160.242` appear as coming from the IP addresses `10.10.0.1`, `10.10.0.2`, and then `ME` to indicate that your IP address should appear in the third order. Another example command would be `nmap -D 10.10.0.1,10.10.0.2,RND,RND,ME 10.65.160.242`, where the third and fourth source IP addresses are assigned randomly, while the fifth source is going to be the attacker’s IP address. In other words, each time you execute the latter command, you would expect two new random IP addresses to be the third and fourth decoy sources.

---------------------------------------------------------------------------------------

#### What do you need to add to the command sudo nmap 10.65.160.242 to make the scan appear as if coming from the source IP address 10.10.10.11 instead of your IP address?

Answer: `-S 10.10.10.11`

#### What do you need to add to the command sudo nmap 10.65.160.242 to make the scan appear as if coming from the source IP addresses 10.10.20.21 and 10.10.20.28 in addition to your IP address?
...
![IP Packet Header](Images/IP_Packet_Header.png)
...
![Nmap Fragmentation 1](Images/Nmap_Fragmentation_1.png)
...
![Nmap Fragmentation 2](Images/Nmap_Fragmentation_2.png)
...
The idle scan, or zombie scan, requires an idle system connected to the network that you can communicate with. Practically, Nmap will make each probe appear as if coming from the idle (zombie) host, then it will check for indicators whether the idle (zombie) host received any response to the spoofed probe. This is accomplished by checking the IP identification (IP ID) value in the IP header. You can run an idle scan using `nmap -sI ZOMBIE_IP 10.65.160.242`, where `ZOMBIE_IP` is the IP address of the idle host (zombie).
...
![Nmap Idle Scan](Images/Nmap_Idle_Scan.png)
...
![Nmap Idle Scan Closed](Images/Nmap_Idle_Scan_Closed.png)
...
![Nmap Idle Scan Open](Images/Nmap_Idle_Scan_Open.png)
...
Launch the AttackBox if you haven't done so already. After you make sure that you have terminated the VM from Task 4, start the VM for this task. Wait for it to load completely, then open the terminal on the AttackBox and use Nmap with `nmap -sS -F --reason 10.64.142.139` to scan the VM.

---------------------------------------------------------------------------------------

#### What is the reason provided for the stated port(s) being open?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Medium/Nmap_Advanced_Port_Scans]
└─$ nmap -sS -F --reason 10.64.142.139
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-15 09:32 CET
Nmap scan report for 10.64.142.139
...
|TCP Xmas Scan|`sudo nmap -sX 10.64.142.139`|
|TCP Maimon Scan|`sudo nmap -sM 10.64.142.139`|
|TCP ACK Scan|`sudo nmap -sA 10.64.142.139`|
|TCP Window Scan|`sudo nmap -sW 10.64.142.139`|
|Custom TCP Scan|`sudo nmap --scanflags URGACKPSHRSTSYNFIN 10.64.142.139`|
|Spoofed Source IP|`sudo nmap -S SPOOFED_IP 10.64.142.139`|
|Spoofed MAC Address|`--spoof-mac SPOOFED_MAC`|
|Decoy Scan|`nmap -D DECOY_IP,ME 10.64.142.139`|
|Idle (Zombie) Scan|`sudo nmap -sI ZOMBIE_IP 10.64.142.139`|
