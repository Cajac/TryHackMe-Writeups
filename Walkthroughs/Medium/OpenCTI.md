...
![OpenCTI Logo](Images/OpenCTI_Logo.png)
...
![OpenCTI Web Site](Images/OpenCTI_Web_Site.png)
...
![OpenCTI Data Model](Images/OpenCTI_Data_Model.png)
...
Follow along with the task by launching the attached machine and using the credentials provided; log in to the OpenCTI Dashboard via the AttackBox on `http://10.10.206.90:8080/`. Give the machine **10 minutes to start up**, and using the AttackBox on fullscreen is advisable.
...
![OpenCTI Dashboard](Images/OpenCTI_Dashboard.gif)
...
![OpenCTI Analysis](Images/OpenCTI_Analysis.gif)
...
![OpenCTI Events](Images/OpenCTI_Events.gif)
...
![OpenCTI Observations](Images/OpenCTI_Observations.gif)
...
![OpenCTI Threats](Images/OpenCTI_Threats.png)
...
![OpenCTI Arsenal](Images/OpenCTI_Arsenal.gif)
...
![OpenCTI Entities](Images/OpenCTI_Entities.gif)

#### What is the name of the group that uses the 4H RAT malware?

Hint: Search for the 4H RAT malware as described above in the Arsenal section.
...
Hint: Find the attack pattern under the Arsenal tab; check its Details section for the Kill chain Phase.
...
![OpenCTI Cobalt Strike Overview](Images/OpenCTI_Cobalt_Strike_Overview.png)
...
![OpenCTI Cobalt Strike Knowledge](Images/OpenCTI_Cobalt_Strike_Knowledge.gif)
...
![OpenCTI Cobalt Strike Analysis](Images/OpenCTI_Cobalt_Strike_Analysis.png)
...
1. Search for `Cobalt Strike` under `Malware` in `Arsenal`
2. Select the `Knowledge` Tab in the top menu
3. Select `Intrusion Sets` in the right menu
4. Sort (twice) on the `Confidence` column

Answer: CopyKittens, FIN7

#### Who is the author of the entity?

Go back to the `Overview` tab and check `Author` under `Basic Information`
...
Hint: ESET CaddyWiper March 2022 report

Answer: 2022/03/15

#### Which Attack technique is used by the malware for execution?

Hint: Attack Pattern details under the Knowledge Tab of the entity

Answer: Native API

#### How many malware relations are linked to this Attack technique?

Hint: See Distribution of Relations details under the Knowledge Tab of the technique

Answer: 113

#### Which 3 tools were used by the Attack Technique in 2016? (Ans: Tool1, Tool2, Tool3)

Hint: Look under Knowledge -> Tools. List tools in alphabetical order.

Answer: BloodHound, Empire, ShimRatReporter

#### What country is APT37 associated with?

Hint: Check Description of the Intrusion set

1. Select `Threats` in the left-most menu and then the `Intrusion sets` tab in the top menu
2. Search for `APT37`
3. Check Description text under Details to the right
