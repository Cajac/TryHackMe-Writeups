...
In some cases, the system’s hibernation file (hiberfil.sys) can also be parsed to extract RAM contents saved when the machine enters hibernation mode. More information can be found in this article.
...
#### What technique involves replacing a trusted process’s memory with malicious code?
...
`https://static-labs.tryhackme.cloud/apps/memory-analysis-intro/`
...
- [Random-access memory - Wikipedia](https://en.wikipedia.org/wiki/RAM_memory)
