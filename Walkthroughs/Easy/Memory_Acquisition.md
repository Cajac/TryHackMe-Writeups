...
Hint: This tool produces a .core file.
...
In this room, we will use a Windows Server 2019 virtual machine with FTK Imager and SysInternalsSuite installed. You can start the machine by clicking the **Start Machine** button. The VM will take approximately 2 minutes to boot up and will start in split view. If the VM is not visible, use the blue **Show Split View** button at the top of the page.

Alternatively, you can connect to the VM via Remote Desktop (RDP) using the credentials below:

- Username: Administrator
- Password: TryH@cKMe9#21
- IP-addres: 10.10.246.10
...
![FTK Images Full Memory Capture](Images/FTK_Images-Full_Memory_Capture.gif)
...
![Windows Configure Crash Dump](Images/Windows_Configure_Crash_Dump.gif)

---------------------------------------------------------------------------

Start notepad.exe on the VM and use the procdump64.exe tool to write a 'triage' dump file of the process. Ensure that the dump file's name is formatted like PROCESSNAME_PID_YYMMDD_HHMMSS.dmp.

#### Enter the complete command below

Hint: Use .\procdump64.exe -h to find the correct flag and formatting

Note: Use PowerShell so to syntax is correct. No need to include the -accepteula flag

Answer: .\procdump64.exe -mt notepad.exe PROCESSNAME_PID_YYMMDD_HHMMSS.dmp

#### Which two tools can you use to extract or dump memory artifacts of the lsass.exe process?

Hint: One is a malicious tool, the other is part of the SysInternals suite.

Enter the answers in alphabetic order, separated by a comma, and in the same format they are mentioned in the task.

Answer: Mimikatz,procdump64.exe

### Task 4 - Memory Acquisition on Linux

#### Start Virtual Machine

In this room, we will use a Ubuntu desktop VM. You can start the machine by clicking the **Start Machine** button. The VM will take approximately 1 minute to boot up and will start in split view. If the VM is not visible, use the blue **Show Split View** button at the top of the page.
...
Hint: Consult the LiME help page referenced in this task

Consult the help page at: `https://github.com/504ensicsLabs/LiME/tree/master/doc`

Answer: sudo insmod lime-6.8.0-1027-aws.ko "path=tcp:5555 format=raw"
...
Consult the page at: `https://github.com/504ensicsLabs/LiME`

Answer: sudo insmod lime-6.8.0-1027-aws.ko "path=/tmp/memdump.lime format=lime "digest=md5
...
1. Login into the VM and use the techniques seen in the previous tasks
2. Use the built-in tools of the hypervisor or cloud platform to extract memory without interacting directly with the VM. This can be done via the GUI or the command line
...
![vSphere Memory Capture](Images/vSphere_Memory_Capture.gif)
...
Answer: anti-forensic techniques
...
- [ulimit - Linux manual page](https://man7.org/linux/man-pages/man1/ulimit.1p.html)
