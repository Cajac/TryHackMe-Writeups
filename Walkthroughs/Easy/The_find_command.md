...
Room link: [https://tryhackme.com/room/thefindcommand](https://tryhackme.com/room/thefindcommand)
...
If you don't specify where to look, by default `find` will search the **current working directory recursively** (which means the working directory and all its subdirectories). Also, you can use wildcards, in specifying both a directory and a name.
...
To test `find`, create a directory called "test-dir", enter it, and open a terminal **inside that directory**. On your terminal, execute the command:
...
**Note**: We didn't tell `find` where to look, so by default it searches our working directory.
...
Two very useful flags are the `-type` and `-name` flags. With `-type`, you can use `d` to only find directories, and `f` to only find files. The `-name` flag is used to specify a name or pattern to look for. You can type the whole name, or use wildcards to specify only part(s) of the name.

**Note**: When using the `-name` flag (either searching for exact name or using wildcards), you always need to **enclose your argument in quotes**, otherwise the command won't work as intended.

It is useful to know that you can also use the `-iname` flag; same as `-name`, but case insensitive.
