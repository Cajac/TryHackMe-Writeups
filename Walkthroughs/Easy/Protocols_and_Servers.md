...
Room link: [https://tryhackme.com/room/protocolsandservers](https://tryhackme.com/room/protocolsandservers)
...
![Telnet Wireshark Password](Images/Telnet_Wireshark_Password.png)
...
![HTTP Requests and Responses](Images/HTTP_Requests_and_Responses.png)
...
1. First, we connect to port 80 using `telnet 10.66.185.9 80`.
...
```bash
pentester@TryHackMe$ telnet 10.66.185.9 80
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
...
Launch the attached VM. From the AttackBox terminal, connect using Telnet to 10.66.185.9 80 and retrieve the file flag.thm.

#### What does it contain?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/Protocols_and_Servers]
└─$ telnet 10.66.185.9 80 
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
GET /flag.thm HTTP/1.0

HTTP/1.1 200 OK
Server: nginx/1.18.0 (Ubuntu)
Date: Tue, 16 Dec 2025 06:37:58 GMT
...
```bash
pentester@TryHackMe$ telnet 10.66.185.9 21
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
220 (vsFTPd 3.0.3)
...
![FTP Control and Data Channels](Images/FTP_Control_and_Data_Channels.png)
...
```bash
pentester@TryHackMe$ ftp 10.66.185.9
Connected to 10.66.185.9.
220 (vsFTPd 3.0.3)
Name: frank
331 Please specify the password.
...
Using an FTP client, connect to the VM and try to recover the flag file.
...
Using binary mode to transfer files.
ftp> ls
229 Entering Extended Passive Mode (|||41795|)
150 Here comes the directory listing.
...
226 Directory send OK.
ftp> mget ftp*
mget ftp_flag.thm [anpqy?]? y
229 Entering Extended Passive Mode (|||57772|)
150 Opening BINARY mode data connection for ftp_flag.thm (39 bytes).
100% |****************************************************************************************************************************************************************|    39       17.35 KiB/s    00:00 ETA
226 Transfer complete.
39 bytes received in 00:00 (0.33 KiB/s)
ftp> quit
221 Goodbye.
...
![SMTP Mail Agents](Images/SMTP_Mail_Agents.png)
...
```bash
pentester@TryHackMe$ telnet 10.66.185.9 25
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
...
Using the AttackBox terminal, connect to the SMTP port of the target VM.

#### What is the flag that you can get?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/Protocols_and_Servers]
└─$ telnet 10.66.185.9 25
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
220 bento.localdomain ESMTP Postfix THM{<REDACTED>}
helo mail.cajac.thm
250 bento.localdomain
quit
221 2.0.0 Bye
...
![SMTP Mail Agents 2](Images/SMTP_Mail_Agents_2.png)
...
```bash
pentester@TryHackMe$ telnet 10.66.185.9 110
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
+OK 10.66.185.9 Mail Server POP3 Wed, 15 Sep 2021 11:05:34 +0300 
USER frank
+OK frank
PASS D2xc9CgD
...
.
QUIT
+OK 10.66.185.9 closing connection
Connection closed by foreign host.
```
...
Connect to the VM (10.66.185.9) at the POP3 port. Authenticate using the username frank and password D2xc9CgD.

#### What is the response you get to STAT?

```bash
┌──(kali㉿kali)-[/mnt/…/TryHackMe/Walkthroughs/Easy/Protocols_and_Servers]
└─$ telnet 10.66.185.9 110
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
+OK Hello there.
...
#### How many email messages are available to download via POP3 on 10.66.185.9?
...
```bash
pentester@TryHackMe$ telnet 10.66.185.9 143
Trying 10.66.185.9...
Connected to 10.66.185.9.
Escape character is '^]'.
