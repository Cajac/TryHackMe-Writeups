...
Room link: [https://tryhackme.com/room/introtodockerk8pdqk](https://tryhackme.com/room/introtodockerk8pdqk)
...
Hint: Remember that you specify the tag with the colon key (:)
...
Hint: We can use the `-p` tag for this, how would you tell the container to bind a port? An example of this has been given in the task.
...
Hint: You will need to use docker ps for this with an argument. The answer for this has been given in the task.
...
![Apache Service Running](Images/Apache_Service_Running.png)
...
Hint: This instruction must be placed at the start of the Dockerfile

Answer: `FROM`

#### What instruction would we use to tell the container to run a command?

Hint: This instruction can be used multiple times in a Dockerfile. There is another instruction that is simiar, but is often one of the very last instructions in a Dockerfile.
...
Hint: This is short for "tag". Remember to include "docker build", as this is an argument for the command.
...
![Docker Compose Example 1](Images/Docker_Compose_Example_1.png)
...
![Docker Compose Example 2](Images/Docker_Compose_Example_2.png)
...
Note: These are just some of the possible instructions possible. Check out the [compose file documentation](https://docs.docker.com/compose/compose-file/) for all possible instructions.
...
Hint: It is not start! Remember, this is docker-compose, which uses different syntax to the regular Docker program.

Answer: `up`

#### I want to use docker-compose to delete the series of containers. What argument allows me to do this?

Hint: It is not stop! Remember, this is docker-compose, which uses different syntax to the regular Docker program.

Answer: `down`

#### What is the name of the .yml file that docker-compose uses?

Hint: It is the name of the program! There is a heading in this task that explains this.
...
![Docker Socket](Images/Docker_Socket.png)
...
Hint: This term is commonly used when web applications talk to each other
...
Hint: Look at the name column when using the command to list running Docker containers column.

```bash
cmnatic@thm-intro-to-docker:~$ docker ps
CONTAINER ID   IMAGE         COMMAND               CREATED       STATUS              PORTS     NAMES
0f24bed8b5ef   cloudisland   "tail -f /dev/null"   3 years ago   Up About a minute             CloudIsland
cmnatic@thm-intro-to-docker:~$ 
```
...
```bash
cmnatic@thm-intro-to-docker:~$ docker image ls
REPOSITORY    TAG       IMAGE ID       CREATED       SIZE
webserver     latest    1a2769751f0a   3 years ago   223MB
<none>        <none>    5bc250eb793f   3 years ago   223MB
<none>        <none>    5af755feff52   3 years ago   223MB
<none>        <none>    68a438a5ade0   3 years ago   223MB
cloudisland   latest    eae73f61ae73   3 years ago   77.8MB
ubuntu        22.04     2dc39ba059dc   3 years ago   77.8MB
ubuntu        latest    2dc39ba059dc   3 years ago   77.8MB
cmnatic@thm-intro-to-docker:~$ docker run -p 80:80 webserver
AH00558: apache2: Could not reliably determine the server's fully qualified domain name, using 172.17.0.3. Set the 'ServerName' directive globally to suppress this message

```

After starting the container, try to connect to `https://10-112-132-82.reverse-proxy.cell-prod-eu-central-1a.vm.tryhackme.com/` in your browser.
